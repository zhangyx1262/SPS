...
class AddEditwarehousing extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { warehousingObj, warehousingIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "warehousing") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savewarehousing(values); //保存主表数据
                //返回
    ...
    render() {
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            ...getFieldProps('wh_no', {
                ...
                initialValue: (typeof rowData === 'undefined' || typeof rowData.wh_no === 'undefined') ? "" : rowData.wh_no
,
                rules: [{
                    type:'string',required: true,pattern:/\S+/ig, message: '请输入入库编号',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ wh_no: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('wh_no')}/>
                                </FormItem>
                                <FormItem>
        <Label className="mast">
            质检编号
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        <Select disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            {
            ...getFieldProps('whstate', {
                initialValue: (typeof rowData === 'undefined' || typeof rowData.whstate === 'undefined') ? "" : String(rowData.whstate)
,
                rules: [{
                    required: true, message: '请选择入库状态',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ whstate: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        <FormError errorMsg={getFieldError('whstate')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditwarehousing);
