...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "warehousing", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {warehousingObj, warehousingIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = warehousingObj.list[warehousingIndex] || {};
    ...
    onPrint = () => {
        const {warehousingIndex, warehousingObj} = this.props;
        const {list} = warehousingObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[warehousingIndex] || {};
        actions.masterDetailMany.printDocument({
            queryParams: {
                funccode: 'warehousing',
                nodekey: 'warehousing'
            },
            printParams: {id: id}
        });
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "warehousing") { // 主表
                const {warehousingIndex} = this.props;
                const record = list[warehousingIndex];
                await actions.masterDetailMany.delwarehousing(record);
            }
        }
    }
    /**
    ...
    warehousingColumn = [
        ...
        {
            title: "入库状态",
            dataIndex: "whstateEnumValue",
            key: "whstateEnumValue",
            width: 150,
                render(text,record,index){
                    return record['whstateEnumValue'];
                }
        },
    ];
    ...
    render() {
        ...
        let selectRow = warehousingObj['list'][warehousingIndex] || {};
        // 主表数据为空
        const warehousingForbid = warehousingObj.list.length > 0 ? false : true;
        let {list} = warehousingObj;
        const {id} = list[warehousingIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='入库'/>
                <SearchArea warehousingObj={warehousingObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="warehousing">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('warehousing', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            ...
                            onClick={() => _this.onShowMainModal("warehousing", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                        ...
                        <Button
                            ...
                            shape='border'
                            disabled={warehousingForbid}
                            onClick={() => _this.onClickDel("warehousing")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="warehousing"
                    data={warehousingObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.warehousingColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({warehousingIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            warehousingObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return warehousingIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(warehousingObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "warehousingObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "warehousingObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={warehousingForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
