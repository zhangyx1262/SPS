...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getwarehousing(param));  // 调用 getList 请求数据
            const {data: reswarehousing} = result;
            const {content = []} = reswarehousing || {};

            if (content.length > 0) { // 获取子表数据
                const warehousingObj = structureObj(reswarehousing, param);
                actions.masterDetailMany.updateState({warehousingObj}); // 更新主表数据
            } else {
                 const {
                    warehousingObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        warehousingObj: initStateObj(warehousingObj),
                        warehousingRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, warehousingIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savewarehousing(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savewarehousing(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatewarehousing(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.warehousingObj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async delwarehousing(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delwarehousing([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
         ...
         * @param {*} getState
         */
        async querywarehousing(param, getState) {
