...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getrl(param));  // 调用 getList 请求数据
            const {data: resrl} = result;
            const {content = []} = resrl || {};

            if (content.length > 0) { // 获取子表数据
                const rlObj = structureObj(resrl, param);
                actions.masterDetailMany.updateState({rlObj}); // 更新主表数据
            } else {
                 const {
                    rlObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        rlObj: initStateObj(rlObj),
                        rlRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, rlIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async saverl(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.saverl(param), '添加成功');
