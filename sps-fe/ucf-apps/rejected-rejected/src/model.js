...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getrejected(param));  // 调用 getList 请求数据
            const {data: resrejected} = result;
            const {content = []} = resrejected || {};

            if (content.length > 0) { // 获取子表数据
                const rejectedObj = structureObj(resrejected, param);
                actions.masterDetailMany.updateState({rejectedObj}); // 更新主表数据
            } else {
                 const {
                    rejectedObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        rejectedObj: initStateObj(rejectedObj),
                        rejectedRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, rejectedIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async saverejected(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.saverejected(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updaterejected(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.rejectedObj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async reject(param, getState) {
            const {id} = param;
            const {result}=processData(await api.reject([{id}]), '退货成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {rejectedObj} = getState().masterDetailMany;
         ...
         * @param {*} getState
         */
        async delrejected(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delrejected([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {rejectedObj} = getState().masterDetailMany;
         ...
         * @param {*} getState
         */
        async queryrejected(param, getState) {
