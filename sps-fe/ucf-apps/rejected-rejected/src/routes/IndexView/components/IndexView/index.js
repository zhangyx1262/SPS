...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "rejected", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {rejectedObj, rejectedIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = rejectedObj.list[rejectedIndex] || {};
    ...
    onPrint = () => {
        const {rejectedIndex, rejectedObj} = this.props;
        const {list} = rejectedObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[rejectedIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "rejected") { // 主表
                const {rejectedIndex} = this.props;
                const record = list[rejectedIndex];
                await actions.masterDetailMany.delrejected(record);
            }
        }
    }
    /**
    ...
    rejectedColumn = [
        ...
        {
            title: "退货状态",
            dataIndex: "rjstateEnumValue",
            key: "rjstateEnumValue",
            width: 150,
                render(text,record,index){
                    return record['rjstateEnumValue'];
                }
        },
    ];
    ...
    render() {
        ...
        let selectRow = rejectedObj['list'][rejectedIndex] || {};
        // 主表数据为空
        const rejectedForbid = rejectedObj.list.length > 0 ? false : true;
        let {list} = rejectedObj;
        const {id} = list[rejectedIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='退货'/>
                <SearchArea rejectedObj={rejectedObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="rejected">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('rejected', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            ...
                            onClick={() => _this.onShowMainModal("rejected", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                        ...
                        <Button
                            ...
                            shape='border'
                            disabled={rejectedForbid}
                            onClick={() => _this.onClickDel("rejected")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="rejected"
                    data={rejectedObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.rejectedColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({rejectedIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            rejectedObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return rejectedIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(rejectedObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "rejectedObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "rejectedObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={rejectedForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
