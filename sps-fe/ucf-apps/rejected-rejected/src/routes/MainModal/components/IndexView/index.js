...
class AddEditrejected extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { rejectedObj, rejectedIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "rejected") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.saverejected(values); //保存主表数据
                //返回
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('rj_no')}/>
                                </FormItem>
                                <FormItem>
        <Label className="mast">
            质检单编号
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        <FormError errorMsg={getFieldError('rjstate')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditrejected);
