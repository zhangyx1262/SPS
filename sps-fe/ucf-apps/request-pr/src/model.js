...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getpr(param));  // 调用 getList 请求数据
            const {data: respr} = result;
            const {content = []} = respr || {};

            if (content.length > 0) { // 获取子表数据
                const prObj = structureObj(respr, param);
                actions.masterDetailMany.updateState({prObj}); // 更新主表数据
            } else {
                 const {
                    prObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        prObj: initStateObj(prObj),
                        prRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, prIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savepr(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savepr(param), '添加成功');
         ...
         * @param {*} getState
         */
        async subpr(param, getState) {
            const {id} = param;
            const {result}=processData(await api.subpr([{id}]), '提交成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {prObj} = getState().masterDetailMany;
                const {pageSize} = prObj;
         ...
         * @param {*} getState
         */
        async delpr(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delpr([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {prObj} = getState().masterDetailMany;
