...
class AddEditreq_order extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { req_orderObj, req_orderIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "req_order") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savereq_order(values); //保存主表数据
                //返回
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('ro_no')}/>
                                </FormItem>
                                <FormItem>
        <Label className="mast">
            审核编号
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('rl_no')}/>
                                </FormItem>
                                <FormItem>
        <Label className="mast">
            采购状态
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        <FormError errorMsg={getFieldError('postate')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditreq_order);
